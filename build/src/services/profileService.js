...
class ProfileService {
    ...
    async getProfileFields() {
        try {
            const profiles = await Profile_1.ProfileModel.find().select("name description skills education certifications github linkedin image userId -_id");
            return profiles;
        }
        catch (error) {
            throw new Error(error.message);
    ...
    async queryProfiles() {
        try {
            const profiles = await Profile_1.ProfileModel.find().select("name description skills education certifications github linkedin image userId -_id").populate("userId", "username email -_id");
            return profiles;
        }
        catch (error) {
            throw new Error(error.message);
    ...
    //-----------------------------------------------------------------------------------------------//
    async searchProfiles(keyword, page = 1, pageSize = 10) {
        try {
